...
export default class TaskDB {
  ...
  upsertIssue(issue, repositorySlug) {
    ...
    if (!priorityNum) {
      const max =
        this.db.prepare("SELECT MAX(priority_number) AS m FROM issues").get().m || 0;
      priorityNum = max + 1;
    }
  ...
  reorderTask(id, direction) {
    ...
    if (direction === "up") {
      ...
      if (!neighbor) {
        ...
        this.db.prepare("UPDATE issues SET priority_number=? WHERE id=?").run(
          newVal,
          current.id
        );
        return true;
  ...
  setSprint(id, sprint) {
    this.db.prepare("UPDATE issues SET sprint = ? WHERE id = ?").run(sprint, id);
  }

  setPriority(id, priority) {
    this.db.prepare("UPDATE issues SET priority = ? WHERE id = ?").run(priority, id);
  }

  setStatus(id, status) {
    this.db.prepare("UPDATE issues SET status = ? WHERE id = ?").run(status, id);
  }

  setDependencies(id, deps) {
    this.db.prepare("UPDATE issues SET dependencies = ? WHERE id = ?").run(deps, id);
  }

  setBlocking(id, blocks) {
    this.db.prepare("UPDATE issues SET blocking = ? WHERE id = ?").run(blocks, id);
  }

  setProjectByGithubId(githubId, project) {
    this.db.prepare("UPDATE issues SET project = ? WHERE github_id = ?").run(
      project,
      githubId
    );
  }
  ...
  setSprintByGithubId(githubId, sprint) {
    this.db.prepare("UPDATE issues SET sprint = ? WHERE github_id = ?").run(
      sprint,
      githubId
    );
  }
  ...
  getActivity() {
    return this.db.prepare("SELECT * FROM activity_timeline ORDER BY id DESC").all();
  }
  ...
  getPairById(id) {
    return this.db.prepare("SELECT * FROM chat_pairs WHERE id = ?").get(id);
  }
  ...
  renameProject(oldProject, newProject) {
    ...
    // Update issues
    this.db.prepare("UPDATE issues SET project=? WHERE project=?").run(newProject, oldProject);
  }
}
```