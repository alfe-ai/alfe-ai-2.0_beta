...
export default class TaskDB {
  ...
  _init() {
    ...
    // Add inactive column for archiving chat tabs
    try {
      this.db.exec(`ALTER TABLE chat_tabs ADD COLUMN inactive INTEGER DEFAULT 0;`);
      console.debug("[TaskDB Debug] Added inactive column to chat_tabs.");
    } catch(e) {
      console.debug("[TaskDB Debug] Possibly column exists. Skipped:", e.message);
    }
  ...
  createChatTab(name) {
    ...
    const { lastInsertRowid } = this.db.prepare(`
      INSERT INTO chat_tabs (name, created_at, inactive)
      VALUES (@name, @created_at, 0)
    `).run({
      name,
      created_at: ts
    });
  ...
  listChatTabs() {
    ...
    return this.db.prepare("SELECT * FROM chat_tabs WHERE inactive=0 ORDER BY id ASC").all();
  }
  ...
  closeChatTab(tabId) {
    this.db.prepare("UPDATE chat_tabs SET inactive=1 WHERE id=?").run(tabId);
  }
